		...
		*/
		...
		while (++depth <= MAX_PLY && /*!Signals.stop && 一時的にコメントアウト2015/5/9*/ (!Limits.depth || depth <= Limits.depth))
		{
			...
			*/
			...
			/*
			use_time_management関数は探索に制限を掛けるか決める関数、
				- mate check mateを探索するオプションが掛っている,このすぐ上にあるコードがLimits.mateをチエックしているものである
				- movetime 時間制限が掛っているオプションで探索時間制御とは別のもの
				- depth 探索深さ制限のオプションが掛っている（深さ制限があるので時間制御をダブルでかけることはない）
				- nodes	展開ノード数制限のオプションが掛っている
				- infinite	まったく制限を掛けないことを示すオプション
			これらフラグ全てがfalseになっていなければ制限がかかることはない
			つまりこのif文はオプションによる探索制限が掛っていない時に探索時間制御をおこなう
			*/
			if (Limits.use_time_management() && !Signals.stop && !Signals.stopOnPonderhit)
			{
				/*
				ローカル変数でSignals.stopとは違うので間違わないように
				*/
				...
				*/
				...
				/*
				探索経過時間が最適探索時間の62%を超えたらstopをかける
				なぜ62%
				*/
				...
				/*
				DBL_EPSILON	(浮動小数点の最小値)
				http://blog.goo.ne.jp/yamadokoro/e/3a47c16e9363e29265e8dba8f07dffc5
					- 反復深化深さが12以上
					- BestMoveChangesがほぼ0
					- 別条件でstopがかかっていない（すでにstopが立っているならここはパス）
					- PVSizeが1であること　？
					- bestValueがひどい点数以上あること->ひどい点数のまま探索停止してはだめでしょ
					- ルート局面で合法手が１しかない
					- 探索経過時間が最適探索時間の20%を超えた
				つまりルート局面での合法手が１つしかなく最善手変化もほぼ0でこの手しかないようなので探索中止する方向であるが
				でもこの最善と思われる手(RootMoves[0].pv[0])を回避手指定(ss->excludedMove)にして探索窓を
				最善と思われる手の評価値よりもPawn２個分ハードルを下げて探索してそれでも悪い評価しか出ないようであれば
				現在上がっている手を最善手として返すので探索を中止する
				*/
				if (    depth >= 12
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) 
    ...
    // we should also update RootMoveList to avoid bogus output.
		...
		トランスポジションテーブル（以下tt）に有効な手が登録してあるので使用する
			- ttに登録してある指し手の残り深さと現在の残り深さが小さい＝現在の局面の方が深いところを探索している、トランスポジションテーブルに登録されている手が浅いところで同一局面を検出している
			- ttに登録してある手の評価値がVLAUE_NONE(あり得ない評価値？）ではないこと
			- PvNodeである　＆＆　ttに登録してある手の評価値の種別がBOUND_EXACT(真値)であること
			- NonPvNodeである　＆＆　ttに登録してある手の評価値の種別がBOUND_LOWER(lower失敗)であること
			- NonPvNodeである　＆＆　ttに登録してある手の評価値の種別がBOUND_UPPER(hight失敗)であること
			  BOUND_NONEでなかった（手は登録してあるが探索が終了しておらず評価が確定していない）
			*/
		if (!RootNode
        && tte
        && tte->depth() >= depth
		...
		if (inCheck)
    {
		...
		/*
		王手がかかっていなくて定跡手があり、その評価値がVALUE_NON(とりあえずttに手だけ登録しておいて評価値が確定してない時の評価値定数)
		なら現局面の評価値をevalとss->staticEvalに与える。
		*/
		else if (tte)
    {
        // Never assume anything on values stored in TT
			/*
			ttに登録してある手の評価値が未確定（VALUE_NONE）なら評価関数を読んで現局面の評価値を確定する
			*/
    ...
    // Step 10. Internal iterative deepening (skipped when in check)
		...
		多重反復深化（王手が掛っている時はスキップされる）
		- 残り探索深さがPVNodeなら5よりおおきいこと、非PvNodeなら８よりおおきいこと　つまり末端では多重深化はするな
		- この居面で有効な手がトランスポジションテーブルに登録されていなかった
		- PvNode or この局面の静止評価値に256上乗せした評価値がbeta値を超える（βcut）可能性がある
		本格的に探索する前に浅い探索を行い良い手があったらttMoveに登録しておく
		*/
        ...
        && (PvNode || ss->staticEval + Value(256) >= beta))
    {
        ...
        ss->skipNullMove = false;
				/*
				探索を終えて、この局面で有効な手があったならトランスポジションテーブルに登録されているはずなので
				テーブルを調査して、あればttMoveに登録しておく
				*/
        tte = TT.probe(posKey);
...
void check_time() 
{
  ...
  if (Time::now() - lastInfoTime >= 1000)
  {
	...
	/*
	ponder探索中はuciプロトコルからstop || ponderhitで停止するのでここでのチエックは無用
	*/
  if (Limits.ponder)
      return;
	/*
